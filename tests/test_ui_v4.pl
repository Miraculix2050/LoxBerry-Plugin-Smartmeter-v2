...
like($styles, qr/\@media\(max-width:1200px\)\s*\{\.obis-channel-summary,\.obis-channel-grid,\.obis-api-fields\s*\{\s*grid-template-columns:1fr\}[^}]*\.obis-channel-field\s*\{\s*min-width:0;max-width:100%\}/s, "channel configuration stacks before the LoxBerry sidebar can clip its desktop frame");
