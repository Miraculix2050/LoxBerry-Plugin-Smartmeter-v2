...
like($bridge, qr/throttle_log_event\(\\%rate_limited_log_state,/, "bridge rate-limits recurring operational log entries");
like($bridge, qr/"udp-output".*?"error"/s, "bridge classifies recurring UDP output failures as errors");
like($bridge, qr/sub debug_line.*?log_line\(\$message, "debug"\)/s, "bridge keeps complete Debug logging independent of operational throttling");
