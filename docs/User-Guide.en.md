...
Enable **Bridge service enabled** when the MQTT bridge should forward vzLogger MQTT values to the plugin HTTP cache and optional UDP output. The `vzlogger` service itself remains startable in vzLogger mode independently of the bridge. The **Update cycle** controls how often vzLogger publishes meter values by MQTT; the bridge uses the same cycle for HTTP cache writes and UDP sends. The bridge accepts only the `chnN` mappings configured in `vzlogger_channels.json`. Successful UDP cycles are logged only when bridge debugging is enabled; send failures remain visible in the normal bridge log. The MQTT base topic is a shared setting and remains configurable independently of the service buttons.
