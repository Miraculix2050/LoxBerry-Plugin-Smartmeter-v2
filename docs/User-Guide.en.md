...
Bridge, service-control, web-interface, and explicitly requested diagnostic output use native timestamped LoxBerry log sessions named `bridge`, `control`, `webui`, and `diagnostic`. They appear together under **SmartMeter v2 (Plugin Log)** in the central log manager and share its plugin log level. At the default **Error** level, successful actions no longer create empty sessions; Debug also mirrors detailed bridge parsing to `journalctl -u smartmeter-v2-vzlogger-bridge`. LoxBerry's hourly maintenance compresses or truncates plugin logs at 3 MiB or 30 days, deletes compressed files after 60 days, and trims a plugin log directory from more than 100 files to the newest 24 and limits each registered session group to 24 retained entries. **Show log** and **Show control log** open the newest matching session when one is available.
