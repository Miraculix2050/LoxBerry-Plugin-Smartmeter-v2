...
History is stored persistently and exclusively in the current browser profile or browser container in IndexedDB, never on the LoxBerry or another server. It remains available after reload, closing, and reopening without causing additional writes on the LoxBerry. Multiple ordinary tabs using the same browser profile and address share history and display preferences, but each tab requests live data independently. Other browsers, browser profiles, and isolated browser containers maintain independent histories and preferences. Original two-second values are retained for 15 minutes; after connection failures, requests temporarily back off to at most 30 seconds and return to two seconds after the next success. Older values retain their first, last, minimum, and maximum in 10-second buckets up to 2 hours, one-minute buckets up to 24 hours, and 15-minute buckets up to 7 days. Values older than 7 days are deleted. This retains curve shape and power peaks while bounding storage. Collection gaps and decreasing energy counters remain visible as interruptions and new baselines. Changes to identifier, unit, category, or scaling discard only the incompatible channel history; name-only changes retain it. **Clear stored history** deletes local history after confirmation. If browser storage is unavailable or full, live display continues in memory and shows a warning. An existing earlier tab-session cache is imported once. Channel selection, range, energy mode, and background collection are also stored locally; removed UUIDs are dropped from the selection.
