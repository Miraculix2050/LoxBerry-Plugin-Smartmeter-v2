...
The existing HTTP endpoint serves values from these cache files. When HTTP cache is disabled, the bridge removes existing `.data` files, performs no further cache writes, and the endpoint reports that the cache is disabled. `/var/run/shm` is RAM-backed, so an enabled cache does not burden the SD card either. When cache is enabled, the vzLogger page shows cache status, the last update, and a direct link to the cache endpoint. Output always starts with `Last_Update` and `Last_UpdateLoxEpoche`. Each configured channel then contributes only its output key, in ascending channel-number order (`chn0`, `chn1`, ...). Additional values without a channel number follow alphabetically. This keeps the order unambiguous even when several channels use the same OBIS identifier. If UDP is enabled, the bridge sends values to all configured Miniservers on the shared cache/UDP update cycle and in the same order.
