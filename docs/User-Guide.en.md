...
Below the tables, the live page plots values received during the current tab session in one shared chart. On first use it selects signed total active power, or separate import and export power as a fallback, plus the total grid-import and grid-export counters. Import is positive and export is negative; cumulative imported and exported energy remain positive quantities. Energy defaults to the change since the session started and can be switched to absolute meter readings. A compact summary per reading head shows grid import, grid export, grid balance, current grid flow, and the highest observed import and export power. These figures describe only exchange with the grid, not generation, household consumption, self-consumption, or self-sufficiency.
...
History and session figures are stored in the tab's session storage and restored after a reload. Further recording remains scoped to that tab; changes to the generated channel metadata discard stored history that no longer matches it. Channel selection, energy mode, and the background-collection option are stored persistently in the browser; channel UUIDs that are no longer configured are removed from this selection on the next opening. Data requests pause by default while the tab is hidden. **Try to collect data in the background** keeps requesting without redrawing the page on a best-effort basis, but the browser or operating system may throttle or suspend it, and it consumes additional performance and battery power, especially on mobile devices. The page uses the Chart.js version shipped locally with the plugin and loads no third-party library, font, or telemetry.
